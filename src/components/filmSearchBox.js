...
import { addFilm } from '../scripts/api';
...
const findFilm = (title) =>
  fetch(`https://www.omdbapi.com/?apikey=672fc152&s=${title}`)
    .then((resp) => resp.json())
    .then((data) => renderFilms(data));

const addTheFilm = (el) => {
  if (el.target.nodeName.toLowerCase() === 'a') {
    const { imdb, title, year } = el.target.dataset;
    let ttImdb = imdb;
    if (imdb[0] === 't' && ttImdb[1] === 't') ttImdb = imdb.substr(2);
    addFilm(title, year, ttImdb)
      .then(() => {
...
$results.addEventListener('click', addTheFilm);
