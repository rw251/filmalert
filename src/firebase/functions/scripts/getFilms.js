...
const url = {
  ...
  tvfilmapi: [
    'https://www.tv-films.co.uk/api/shows/',
    'https://www.tv-films.co.uk/api/shows2/',
  ],
};
...
const getFilms = (id, date) =>
  rp(request[service](id))
    .catch(() => {
      // Try the backup url
...
const timeToRemoveFrom = () => {
  ...
  return now
    .toISOString()
    .split('T')
    .reduce((date, time) => date + ' ' + time.substr(0, 5));
};
...
const filmModule = (admin, config) => {
  ...
  const insertNewFilms = async (films) => {
    ...
    console.log(
      `${films.length} films inserting in ${batchArray.length} batch${
        batchArray.length !== 1 ? 'es' : ''
      }...`
    );
    batchIndex = 1;
  ...
  return {
    getFilmsCron: (context) => {
      ...
      const dayAfterTomorrowAsShortDate = dayAfterTomorrow
        .toISOString()
        .substr(0, 10);
